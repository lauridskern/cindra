...
interface WorkspaceActionsApi extends Pick<
  SessionActionsContextValue,
  "openProject" | "openWorkspacePicker"
> {
...
function dispatchRuntimeStatus(
  ...
  status: Awaited<ReturnType<typeof desktopClient.getRuntimeStatus>>,
) {
...
function usePromptDraftStore(
  ...
  const movePromptDraft = useCallback(
  ...
  const setPromptDraftPending = useCallback(
    (key: string, isPending: boolean) => {
      setDrafts((current) =>
        setPromptDraftEntryPending(current, key, isPending),
      );
    },
    [],
  );
  ...
  const checkoutBranch = useCallback(
    async (branchName: string) => {
      try {
        ...
        dispatchRuntimeStatus(dispatch, status);
      } catch (error) {
        dispatchUiError(dispatch, error);
        throw error;
      }
...
> {
    ...
    async (workspacePath: string, conversationId: string) => {
      try {
        ...
        if (latestState.transcripts[conversationId] == null) {
          const transcript =
            await desktopClient.loadConversation(conversationId);
  ...
  const startNewChat = useCallback(
    async (workspacePath?: string) => {
      ...
      const targetWorkspacePath =
        workspacePath ?? getCurrentWorkspacePath(state);
      ...
      try {
        ...
        const originConversationId =
          sessionStateRef.current.currentConversationId;
  ...
  const submitPrompt = useCallback(async () => {
    ...
    const draftEntry =
      draftKey == null
        ? null
        : getPromptDraftEntry(draftsRef.current, draftKey);
...
export function SessionProvider({ children }: { children: ReactNode }) {
  ...
  const { ensureWorkspaceIsOpen, openProject, openWorkspacePicker } =
    useWorkspaceActions({
      dispatch,
      sessionStateRef,
      setIsOpeningProject,
