...
export function usePromptDraftStore(
  ...
  function clearPromptDraft(key: string | null) {
    if (key == null) {
      return;
    }
  ...
  function movePromptDraft(fromKey: string | null, toKey: string | null) {
    setDrafts((current) => movePromptDraftEntry(current, fromKey, toKey));
  }

  function setPromptDraftPending(key: string | null, isPending: boolean) {
    if (key == null) {
      return;
    }
