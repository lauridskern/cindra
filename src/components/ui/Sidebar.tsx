...
function SidebarProvider({
  ...
  const [_open, setUncontrolledOpen] = React.useReducer(
        ...
        style={
          {
            ...
            "--sidebar-floating-width": "calc(var(--sidebar-width-icon) + 1rem)",
            "--sidebar-floating-width-with-border":
              "calc(var(--sidebar-width-icon) + 1rem + 2px)",
            "--sidebar-offcanvas-offset": "calc(var(--sidebar-width) * -1)",
            ...style,
  ...
  return (
    ...
    >
      ...
      <div
        ...
        className={cn(
          ...
          variant === "floating" || variant === "inset"
            ? "group-data-[collapsible=icon]:w-(--sidebar-floating-width)"
      ...
      <div
        ...
        data-side={side}
        className={cn(
          "fixed inset-y-0 z-10 flex h-svh w-(--sidebar-width) transition-[left,right,width] duration-200 ease-linear data-[side=left]:left-0 data-[side=left]:group-data-[collapsible=offcanvas]:left-(--sidebar-offcanvas-offset) data-[side=right]:right-0 data-[side=right]:group-data-[collapsible=offcanvas]:right-(--sidebar-offcanvas-offset)",
          variant === "floating" || variant === "inset"
            ? "p-2 group-data-[collapsible=icon]:w-(--sidebar-floating-width-with-border)"
...
function SidebarRail({ className, ...props }: React.ComponentProps<"button">) {
  ...
  return (
    <button
      ...
      className={cn(
        "absolute inset-y-0 z-20 flex w-4 transition-all ease-linear group-data-[side=left]:-right-4 group-data-[side=right]:left-0 after:absolute after:inset-y-0 after:start-1/2 after:w-0.5 hover:after:bg-sidebar-border ltr:-translate-x-1/2 rtl:-translate-x-1/2",
...
function SidebarGroupAction({
  ...
  return useRender({
    ...
    props: mergeProps<"button">(
      {
        className: cn(
          "absolute top-3.5 right-3 flex aspect-square w-5 items-center justify-center rounded-md p-0 text-sidebar-foreground ring-sidebar-ring outline-hidden transition-transform group-data-[collapsible=icon]:hidden after:absolute after:-inset-2 hover:bg-black/5 hover:text-sidebar-accent-foreground dark:hover:bg-white/5 focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0",
          className,
        ),
      },
      props,
